...
type
  ...
  private
    ...
    function GetAutoRunText: Ansistring;
    procedure SetAutoRunText(Value: Ansistring);
  ...
  public
    ...
    procedure Load(Filename : Ansistring; Silent : Boolean);
    procedure Stop(Index : Integer);
    procedure StopByHandle(Script : TScript);
    procedure StopAll(StopSysScripts : Boolean);
    procedure SwitchBot(ScriptName : AnsiString; StopBotScripts : Boolean);
    procedure ProgramEvent(EventName, MatchText : Ansistring; Exclusive : Boolean);
    function TextOutEvent(Text : Ansistring; StartScript : TScript) : Boolean;
    procedure TextEvent(Text : Ansistring; ForceTrigger : Boolean);
    procedure TextLineEvent(Text : Ansistring; ForceTrigger : Boolean);
    procedure AutoTextEvent(Text : Ansistring; ForceTrigger : Boolean);
    function EventActive(EventName : Ansistring) : Boolean;
    procedure ActivateTriggers;
    procedure DumpVars(const SearchName : Ansistring);
    procedure DumpTriggers;
    procedure CountTimerEvent;
    procedure UnCountTimerEvent;

    property Count : Integer read GetCount;
    property LastScript : Ansistring read FLastScript;
    property ActiveBot : Ansistring read FActiveBot;
    property ActiveBotScript   : Ansistring read FActiveBotScript;
    property ScriptMenu : TMenuItem read FScriptMenu write FScriptMenu;
    property ScriptRef : TScriptRef read FScriptRef;
    property ProgramDir: Ansistring read GetProgramDir;

    property Scripts[Index : Integer] : TScript read GetScript; default;
    property AutoRun: TStringList read GetAutoRun;

  published
    property AutoRunText: Ansistring read GetAutoRunText write SetAutoRunText;
  end;
  ...
  private
    ...
    function GetSysConstValue(var CodeRef : Pointer) : Ansistring;
    function ProcessCmd(Cmd : TScriptCmd; var CmdValues : Pointer) : TCmdAction;
    function TriggerExists(Name : Ansistring) : Boolean;
    function CheckTriggers(TriggerList : TList; const Text : Ansistring; TextOutTrigger, ForceTrigger : Boolean; var Handled : Boolean) : Boolean;
    function CreateTrigger(Name, LabelName, Value : Ansistring) : PTrigger;
    function GetProgramDir: Ansistring;
  ...
  public
    ...
    procedure GetFromFile(const Filename : Ansistring; Compile : Boolean);

    function Execute : Boolean; // return true if script was terminated
    procedure DumpVars(SearchName : Ansistring);
    procedure DumpTriggers;
    procedure AddMenu(MenuItem : TObject);
    procedure GotoLabel(L : Ansistring);
    function TextLineEvent(const Text : Ansistring; ForceTrigger : Boolean) : Boolean;
    function AutoTextEvent(const Text : Ansistring; ForceTrigger : Boolean) : Boolean;
    function TextEvent(const Text : Ansistring; ForceTrigger : Boolean) : Boolean;
    function TextOutEvent(const Text : Ansistring; var Handled : Boolean) : Boolean;
    function ProgramEvent(EventName, MatchText : Ansistring; Exclusive : Boolean) : Boolean;
    function EventActive(EventName : Ansistring) : Boolean;
    procedure SetAutoTrigger(Name, Value, Response : AnsiString; LifeCycle : Integer);
    procedure SetTextLineTrigger(Name, LabelName, Value : Ansistring);
    procedure SetTextOutTrigger(Name, LabelName, Value : Ansistring);
    procedure SetTextTrigger(Name, LabelName, Value : Ansistring);
    procedure SetEventTrigger(Name, LabelName, Value, Param : Ansistring);
    procedure SetDelayTrigger(Name, LabelName : Ansistring; Value : Integer);
    procedure KillTrigger(Name : Ansistring);
    procedure KillAllTriggers;
    procedure InputCompleted(InputText : Ansistring; VarParam : TVarParam);
    procedure SetVariable(VarName, Value, Index : Ansistring);
    procedure Gosub(LabelName : Ansistring);
    procedure Return;
    procedure AddWindow(Window : TScriptWindow);
    procedure RemoveWindow(Window : TScriptWindow);
    function FindWindow(WindowName : Ansistring) : TScriptWindow;
    ...
    property WaitText : Ansistring read FWaitText write FWaitText;
    property OutText : Ansistring read FOutText;
    ...
    property ProgramDir: Ansistring read GetProgramDir;
    property ScriptName: Ansistring read GetScriptName;
  end;
...
const
  TriggerNameMap: array[TTriggerType] of Ansistring = ('Text', 'Text-Line', 'Text-Out', 'Delay', 'Event', 'Text-Auto');
...
begin
  ...
  for I := 0 to AutoRun.Count - 1 do
    Load(AnsiString(AutoRun[I]), False);
end;

function TModInterpreter.GetProgramDir: Ansistring;
begin
  Result := FProgramDir;
end;

procedure TModInterpreter.SetProgramDir(const Value: Ansistring);
begin
  FProgramDir := Value;
end;
...
begin
  ProgramEvent('Time hit', AnsiString(TimeToStr(Now)), True);
end;
...
procedure TModInterpreter.Load(Filename : Ansistring; Silent : Boolean);
var
  Script : TScript;
  Error  : Boolean;
  I      : Integer;
begin
  ...
  Filename := AnsiString(StringReplace(String(Filename), '\\', '\', [rfReplaceAll]));
  ...
  SetCurrentDir(String(FProgramDir));
  ...
  if (Copy(UpperCase(String(Filename)), Length(String(Filename)) - 3, 4) = '.CTS') then
  begin
    if not (Silent) then
...
procedure TModInterpreter.SwitchBot(ScriptName : AnsiString; StopBotScripts : Boolean);
var
   I : Integer;
   IniFile     : TIniFile;
   Script,
...
begin
  ...
  begin
    ...
    try
     ExtractStrings([','], [], PChar(String(ScriptName)), ScriptList);
     for sscript in ScriptList do
     script := AnsiString(Sscript);
     begin
...
procedure TModInterpreter.ProgramEvent(EventName, MatchText : Ansistring; Exclusive : Boolean);
var
  I : Integer;
begin
...
function TModInterpreter.TextOutEvent(Text : Ansistring; StartScript : TScript) : Boolean;
var
  I       : Integer;
  Handled : Boolean;
begin
...
procedure TModInterpreter.TextEvent(Text : Ansistring; ForceTrigger : Boolean);
var
  I : Integer;
begin
...
procedure TModInterpreter.TextLineEvent(Text : Ansistring; ForceTrigger : Boolean);
var
  I : Integer;
begin
...
procedure TModInterpreter.AutoTextEvent(Text : Ansistring; ForceTrigger : Boolean);
var
  I : Integer;
begin
  // trigger matching textline triggers in active scripts
...
function TModInterpreter.EventActive(EventName : Ansistring) : Boolean;
var
  I : Integer;
begin
...
procedure TModInterpreter.DumpVars(const SearchName : Ansistring);
var
  I : Integer;
begin
  if (SearchName = '') then
...
function TModInterpreter.GetAutoRunText: Ansistring;
begin
  Result := AutoRun.Text;
end;

procedure TModInterpreter.SetAutoRunText(Value: Ansistring);
begin
  AutoRun.Text := Value;
end;
...
function TScript.GetProgramDir: Ansistring;
begin
  Result := Controller.ProgramDir;
end;

function TScript.GetScriptName: Ansistring;
begin
  Result := ShortFilename(Cmp.ScriptFile);
end;

procedure TScript.GetFromFile(const Filename : Ansistring; Compile : Boolean);
begin
  if (Compile) then
...
function TScript.CheckTriggers(TriggerList : TList; const Text : Ansistring; TextOutTrigger, ForceTrigger : Boolean; var Handled : Boolean) : Boolean;
var
  LifeCycle,
  I         : Integer;
  LabelName,
...
function TScript.TriggerExists(Name : Ansistring) : Boolean;
var
  I: Integer;
  TriggerList: TList;
  TriggerType: TTriggerType;
...
function TScript.TextOutEvent(const Text : Ansistring; var Handled : Boolean) : Boolean;
begin
  FOutText := Text;
...
function TScript.TextLineEvent(const Text : Ansistring; ForceTrigger : Boolean) : Boolean;
var
  Handled : Boolean;
begin
...
function TScript.AutoTextEvent(const Text : Ansistring; ForceTrigger : Boolean) : Boolean;
var
  Handled : Boolean;
begin
...
function TScript.TextEvent(const Text : Ansistring; ForceTrigger : Boolean) : Boolean;
var
  Handled : Boolean;
begin
  // check waitfor
...
function TScript.ProgramEvent(EventName, MatchText : Ansistring; Exclusive : Boolean) : Boolean;
var
  I         : Integer;
...
function TScript.EventActive(EventName : Ansistring) : Boolean;
var
  I : Integer;
begin
...
function TScript.CreateTrigger(Name, LabelName, Value : AnsiString) : PTrigger;
begin
...
function TScript.FindWindow(WindowName : Ansistring) : TScriptWindow;
var
  I : Integer;
begin
...
procedure TScript.SetAutoTrigger(Name, Value, Response : AnsiString; LifeCycle : Integer);
var
  Trigger : PTrigger;
begin
...
procedure TScript.SetTextLineTrigger(Name, LabelName, Value : AnsiString);
begin
  FTriggers[ttTextLine].Add(CreateTrigger(Name, LabelName, Value));
end;

procedure TScript.SetTextOutTrigger(Name, LabelName, Value : AnsiString);
begin
  FTriggers[ttTextOut].Add(CreateTrigger(Name, LabelName, Value));
end;

procedure TScript.SetTextTrigger(Name, LabelName, Value : AnsiString);
begin
  FTriggers[ttText].Add(CreateTrigger(Name, LabelName, Value));
end;

procedure TScript.SetEventTrigger(Name, LabelName, Value, Param : Ansistring);
var
  Trigger : PTrigger;
begin
  Cmp.ExtendName(Name, ExecScriptID);
...
procedure TScript.SetDelayTrigger(Name, LabelName : Ansistring; Value : Integer);
var
  Trigger : PTrigger;
begin
  Cmp.ExtendName(Name, ExecScriptID);
...
procedure TScript.KillTrigger(Name : Ansistring);
var
  I: Integer;
  Found: Boolean;
  TriggerList: TList;
...
procedure TScript.InputCompleted(InputText : Ansistring; VarParam : TVarParam);
begin
...
procedure TScript.SetVariable(VarName, Value, Index : Ansistring);
var
  I       : Integer;
  Param   : TVarParam;
...
procedure TScript.DumpVars(SearchName : Ansistring);
var
  I        : Integer;
  VarParam : TVarParam;
  Sorted   : TList;
...
function TScript.GetSysConstValue(var CodeRef : Pointer) : Ansistring;
var
  ConstRef    : Word;
...
var
  ...
  procedure AppendTempValue(const Value : Ansistring);
  var
    TempParam : TCmdParam;
  begin
...
procedure TScript.GotoLabel(L : Ansistring);
var
  Error : Boolean;
  I     : Integer;
begin
...
procedure TScript.Gosub(LabelName : AnsiString);
begin
