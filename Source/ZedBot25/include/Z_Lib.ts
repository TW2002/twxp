...
CutText CURRENTLINE $promptis2 1 2
If ($promptis2 = "S:") or ($promptis2 = "F:") or ($promptis2 = "M:") or ($promptis2 = "C:")
	Send "**"
	SetVar $ploop ($ploop + 1)
	If ($ploop > 3)
		Goto :BADPROMPT
...
While ($x <= $sortfile)
	Gosub :UPDATEDIAL
	CutText $labels[$x] $sort 1 1
	GetCharCode $sort $sortcode
	If ($sortcode > 0)
		SetVar $bucketct[$sortcode] ($bucketct[$sortcode] + 1)
		SetVar $bucket[$sortcode][$bucketct[$sortcode]] $sortfile[$x]
	End
	SetVar $x ($x + 1)
End
SetVar $z 1
...
While ($z <= 255)
	...
	While ($x <= $bucketct[$z])
		...
		SetVar $sortfile[$index] $bucket[$z][$x]
		SetVar $x ($x + 1)
	End
	SetVar $z ($z + 1)
End
...
# STRINGREPEAT 
#
# SetVar $Z_Lib~repeat to the number of times to repeat the character 
# SetVar $Z_Lib~char to the character to repeat 
# Gosub :Z_Lib~STRINGREPEAT 
# Returns $Z_Lib~string 
:STRINGREPEAT
SetVar $string ""
SetVar $x 1
While ($x <= $repeat)
	SetVar $string ($string & $char)
	SetVar $x ($x + 1)
End
Return
