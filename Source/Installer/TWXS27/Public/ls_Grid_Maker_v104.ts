    ...
    #=--------                                                                       -------=#
	#		Incep Date	:	March 26, 2007 - Version 1.04
	#		Author		:	LoneStar
	...
	setVar $TagVersion 			"Version 1.04"
	...
	if (CONNECTED)
		Echo ANSI_8&"*    0"&ANSI_10&" - "&$LINE0&"Fig Hit Msg's"
	else
		Echo ANSI_8&"*    0"&ANSI_10&" - "&$LINE0&"Fig Hit Msg's     "&ANSI_7&"Must Be Connected!"
	end
	...
	elseif (($Select = "0") AND (CONNECTED))
		gosub :quikstats
		gosub :Reset_Lines
		if (($CURRENT_PROMPT <> "Command") AND ($CURRENT_PROMPT <> "Citadel"))
			echo $TagLineB&" Must Be At Command OR Citadel*"
			goto :Menu_Top_No_Clear
		end
		setVar $RESULT 0
		SetVar $LINE0 $SELECTED
	elseif ($Select = "O")
		#					----------={	List Ordering
   	...
   	#					----------={	Write File
		...
		if ($RESULT <> 0)
			...
			elseif ($LST_ORDER = "Random")
				...
				setVar $Upper_Limit	SECTORS
				setVar $idx 		1
				while (($idx <= SECTORS) AND ($RESULT <= $LIMITOR))
					getRnd $ptr 1 $Upper_Limit
					setVar $ret $ptr
					setVar $temp 0
					while ($ptr <= SECTORS)
						if ($Results[$ptr] <> 0)
...
:Make_List
	...
	elseif ($LINE0 = $SELECTED)
		send " cm"
		waitfor "<Read messages>"
		:txt_Again
		setTextTrigger		txt_Done1	:txt_Done		"Computer command [TL"
		setTextTrigger		txt_Done2	:txt_Done		"No messages received."
		setTextTrigger		txt_Pause1	:txt_Pause		"[Pause]"
		setTextTrigger		txt_Pause2	:txt_Pause		"[Press Space or Enter to continue]"
		:txt_Again2
		setTextLineTrigger	txt_Kill	:txt_Kill		"of your fighters in sector"
		pause
		:txt_Pause
			killAllTriggers
			send "*"
			goto :txt_Again
		:txt_Kill
			setVar $txt_Temp (CURRENTANSILINE & "@@%%^^")
			getWordPos $txt_Temp $pos "[0;32m of"
			if ($pos <> 0)
				getText $txt_Temp $txt_out "sector [1;33m" "@@%%^^"
				stripText $txt_out #13
				stripText $txt_out #10
				setVar $Results[$txt_out] 1
				add $RESULT 1
			end
			goto :txt_Again2
		:txt_Done
