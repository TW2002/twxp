...
:main_menu
     ...
     echo ANSI_5 "*-------------- " ANSI_12 "Dnyarri's macro trigger script" ANSI_5 " --------------*"
     echo ANSI_11 "T" ANSI_9 "rigger text.*"
     echo ANSI_10 "      Currently: " ANSI_15 $trigger_text "*"
     echo ANSI_11 "M" ANSI_9 "acro to run.*"
     echo ANSI_10 "      Currently: " ANSI_15 $macro_text "*"
     echo ANSI_11 "W" ANSI_9 "ait for how long (in seconds)?*"
     echo ANSI_10 "      Currently: " ANSI_15 $timeout_count "*"
     ...
     echo ANSI_11 "R" ANSI_9 "un macro once (single-shot mode).*"
     echo ANSI_11 "P" ANSI_9 "ersistent mode (triggers until timeout).*"
     ...
     if ($chosen_option = "T")
            getinput $trigger_text "Trigger (max 255 chars): "
            goto :main_menu
     end
     ...
     if ($chosen_option = "W")
            getinput $timeout_count "Wait for (in seconds, 0 disables timeout): "
            goto :main_menu
     end
     ...
     if ($chosen_option = "S")
            ...
            if ($file_loaded <> "") AND ($file_loaded <> 0)
                 delete $file_loaded
                 write $file_loaded "MACRO - dnyarri macro trigger"
     ...
     if ($chosen_option = "R") OR ($chosen_option = "P")
           # Check some bounds
           if ($trigger_text = "")
               echo "**" ANSI_12 "You must enter a trigger!*"
               goto :main_menu
           end
           ...
           # Delay is microseconds, convert from seconds...
           setVar $delay_time ($timeout_count * 1000)
           ...
           send "'-==[ Dny trigger started ]==-*"
           goto :wait_around
     end
goto :main_menu

# End of input checking, let's start the clock...
:wait_around
     # Kill any triggers, set new triggers.
     killalltriggers
     if ($timeout_count > 0)
           setDelayTrigger timeout_delay :timed_out $delay_time
     end
# Wait block initialized.

:macro_wait
     setTextTrigger macro_trigger :run_macro $trigger_text
pause

:timed_out
     echo ANSI_12 "**Macro trigger has expired.*"    
goto :script_end

:script_end
     send "'-==[ Dny trigger completed ]==-*"
     # A clean exit.
halt

:run_macro
     # echo ANSI_12 "**Macro trigger hit, running macro...*"
     ...
     if ($persistent > 0)
            # Persistent run, start the macro wait again.
            goto :macro_wait
     end
goto :script_end
