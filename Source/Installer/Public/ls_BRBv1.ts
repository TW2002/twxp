...
:ScriptHeader
	...
	:LetsGetItOn
	send "'" & $TagLineB & " ACTIVE! Firing Photon Into: " & $target & ", On The Hour*"
	getTime $hour "h"
	if ($hour > 12)
		subtract $hour 12
	elseif ($hour = 23)
...
:again
	...
	send " T"
	setTextLineTrigger 1 :Fire1 $hour & ":00:00"
	setTextLineTrigger 2 :Fire1 $hour & ":00:01"
	setTextTrigger 4     :Scan "Computer command [TL="
	:DoAnotherTimeCheck
	setTextLineTrigger 3 :TimeCheck
	pause

	:TimeCheck
	setVar $TCheck CURRENTLINE
	getWordPos $TCheck $pos ":"
	if ($pos <> 0)
		replacetext $TCheck ":" " "
		getWord $TCheck $hrs 1
   		getWord $TCheck $min 2
		getWord $TCheck $sec 3
		isNumber $tst $hrs
		if ($tst <> 0)
#			echo "*" & ANSI_14 & $hrs & "/" & $hour & " " & $min & " " & $sec
	        if (($hrs >= $hour) AND ($min >= 0) AND ($sec >= 3))
	        	killAllTriggers
				echo "**" & ANSI_14 & $TagLineB & ANSI_15 " - Time Appears to have Passed the hour, Shutting Down.*"
				send " Q"
				halt
			end
		end
	...
	send "'" & $TagLineB & " - Bad Sector Data, Photon Not Fired!*"
	goto :bring_me_home

	:gotem
	killAllTriggers
	send "'" & $TagLine & " - Photon Fired --> " & $target & "!* * "
	waitfor "Message sent on sub-space channel"
	if (($holo) AND ($SCAN_TYPE = "Holo"))
		gosub :doScan
	end
	...
	:no_pwarp_lock
		killAllTriggers
		send "'" & $TagLine & " - Unable To Pwarp To " & $start & "*"
		halt

	:pwarp_lock
		killAllTriggers
		waitFor "Planet is now in sector"
		send "'" $TagLine & " - Planet Returned To " & $start & "*"
		goto :Continue
	:already
		killAllTriggers
		send "'" & $TagLine & " - Planet Hasn't Moved From " & $start & "*"
		goto :Continue
	end
...
:TakeMeHome
		...
		:xtwarp_lock
			KillAlltriggers
			send "y * * '" & $TagLineB &  " - Twarp Completed!*"
		:xtwarpDone
            if ($msg <> "")
                send "'" & $TagLineB & " - " & $msg & "*"
            end
